 ...
 */
Adafruit_G77E0_TEA6320::Adafruit_G77E0_TEA6320() : Adafruit_GFX(LCD_WIDTH, LCD_HEIGHT) {}
...
{
	uint8_t initArray[15] =
	{
			0b00000000,		// control byte, Co = 0, D/C = 0
	...
	HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c1, LCD_SLAVE_ADDRESS, initArray, (uint16_t)15, (uint32_t)I2C_DELAY);
//	while(status != HAL_OK);
	clearDisplay();
}
...
void Adafruit_G77E0_TEA6320::display(void) {
	...
	HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c1, LCD_SLAVE_ADDRESS, arrayToWrite, 1 + ALL_LCD_PIXELS, I2C_DELAY);
//	while(status != HAL_OK);
}
...
void Adafruit_G77E0_TEA6320::clearDisplay(void) {
	...
	for(int i = 1; i < ALL_LCD_PIXELS; i++) arrayToClear[i] = 0b00000000;
	setLcdCursorPosition(0, 0);

	HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c1, LCD_SLAVE_ADDRESS, arrayToClear, 1 + ALL_LCD_PIXELS, I2C_DELAY);
//	while(status != HAL_OK);
}
...
void Adafruit_G77E0_TEA6320::setLcdCursorPosition(uint8_t x, uint8_t y) {
	uint8_t setPositionArray[5] =
	{
			0b00000000,		// control byte C0=0,D/C=0;
			0b00000001,		// LCD_COMMAND_DEF_PAGE
			0b00100000,		// set ram page 0
			0b01000000|y,	// set y pos
			0b10000000|x,	// set x pos
	};
	HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(&hi2c1, LCD_SLAVE_ADDRESS, setPositionArray, 5, I2C_DELAY);
	//	while(status != HAL_OK);
}
	...
	for(int i = 0; i < stringLength; i++)
	{
		...
		if (symbol >= 0xC0) {
			...
			case 0xD0: {
				...
				if (symbol == 0x81) { symbol = 0xA8; break; }
				if (symbol >= 0x90 && symbol <= 0xBF) symbol = symbol + 0x30;
				break;
			}
			case 0xD1: {
				symbol = (uint8_t)string[i] - 1;
				if (symbol == 0x91) { symbol = 0xB8; break; }
				if (symbol >= 0x80 && symbol <= 0x8F) symbol = symbol + 0x70;
				break;
			}
			}
		}
...
void Adafruit_G77E0_TEA6320::drawSpectrum(uint16_t * inputArray) {
	...
	for(i = 0; i < FFT_LENGTH / 2; i++)
	{
		...
		fillRect(i * 1, 64 - previousAnalysis[i], 1,  previousAnalysis[i], BLACK);
	}
...
void Adafruit_G77E0_TEA6320::drawFloat(float number) {
	...
	for (uint8_t outputIndex = 0; outputIndex < (index - 2); outputIndex++) {
		println(numbers[aaa[outputIndex]]);
	}

	println(".");

	for (uint8_t outputIndex = index - 2; outputIndex < index; outputIndex++) {
		println(numbers[aaa[outputIndex]]);
	}
}
